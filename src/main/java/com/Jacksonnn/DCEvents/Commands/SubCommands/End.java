...
public class End implements EventSubCommand {
    ...
    public void execute(CommandSender sender, List<String> args) {
        if (args.size() == 1) {
            ...
            if (event == null) {
                sender.sendMessage(GeneralMethods.getErrorPrefix() + "Error! That is not an active event. Check the list to confirm.");
                return;
            }

            event.remove();
            sender.sendMessage(GeneralMethods.getSuccessPrefix() + "Successfully terminated the event.");
        } else {
